...
bool seq_test_and_check(const unsigned long long int NITER, const SpecLib::SpecAtomic<SDouble_T>& sumatorioP) {
	...
	const bool check1 = (sumatorioP.load().getValue() == sumatorioS);
	const bool check2 = ((sumatorioP.load()) == static_cast<IDouble_T>(sumatorioS));
	std::cout << "[*VALIDATION*]: " << (check1 ? "OK!" : (check2 ? "OK! (small precision differences in the acceptable range)" :"FAILED!")) << std::endl;
	if (!check1) {
		const IDouble_T diff = ((sumatorioP.load().getValue() > sumatorioS) ? (sumatorioP.load().getValue() - sumatorioS) : (sumatorioS - sumatorioP.load().getValue()));
		const IDouble_T maxValue = std::abs(std::max(std::abs(sumatorioP.getValue()), std::abs(sumatorioS)));
		...
		std::cout << "ValuePar: " << std::setw(fieldWidth) << std::fixed << std::setprecision(10) << std::right << sumatorioP.load().getValue() << std::endl;
...
auto sf_loop = [](const SpecLib::ExCommonSpecInfo_t& cs, const unsigned long long int begin, const unsigned long long int end, const unsigned long long int step, SpecLib::SpecAtomic<SDouble_T>& sumatorio) {
	IDouble_T lsumatorio = static_cast<IDouble_T>(0.0);
	for (unsigned long long int iteration = begin; (iteration < end) && !cs.cancelled(); iteration += step) {
#ifdef ENABLE_DELAY
		mywait(DelaySeconds);
#endif
		const IDouble_T value = (IDouble_T)(iteration % 2)*static_cast<IDouble_T>(0.0012) + static_cast<IDouble_T>(0.0000003)*(IDouble_T)iteration;
//		sumatorio.fetch_add(value, std::memory_order_relaxed);
		lsumatorio += value;
	}
	sumatorio.fetch_add(lsumatorio, std::memory_order_relaxed);
	//sumatorio += lsumatorio;
};

auto sf = [](const unsigned long long iteration, SpecLib::SpecAtomic<SDouble_T>& sumatorio) {
#ifdef ENABLE_DELAY
	mywait(DelaySeconds);
#endif
	const IDouble_T value = (IDouble_T)(iteration % 2)*static_cast<IDouble_T>(0.0012) + static_cast<IDouble_T>(0.0000003)*(IDouble_T)iteration;
...
int main(int argc, char *argv[]) {
	...
	SpecLib::SpecAtomic<SDouble_T> sumatorio{3.123124};
	...
	/****************************/
	std::cout << "Sumatorio = " << std::fixed << std::setprecision(10) << sumatorio.load(std::memory_order_relaxed) << std::endl;
