...
bool seq_test_and_check(const unsigned long long int NITER, const SDouble_T& sumatorioP) {
	...
	const bool check1 = (sumatorioP.getValue() == sumatorioS);
	const bool check2 = (sumatorioP == static_cast<SDouble_T>(sumatorioS));
	std::cout << "[*VALIDATION*]: " << (check1 ? "OK!" : (check2 ? "OK! (small precision differences in the acceptable range)" : "FAILED!")) << std::endl;
	if (!check1) {
		const IDouble_T diff = ((sumatorioP.getValue() > sumatorioS) ? (sumatorioP.getValue() - sumatorioS) : (sumatorioS - sumatorioP.getValue()));
		...
		std::cout << "ValuePar: " << std::setw(fieldWidth) << std::fixed << std::setprecision(10) << std::right << sumatorioP.getValue() << std::endl;
...
auto sf_loop = [](const SpecLib::ExCommonSpecInfo_t& cs, const unsigned long long int begin, const unsigned long long int end, const unsigned long long int step, SDouble_T& sumatorio) {
	IDouble_T lsumatorio = static_cast<IDouble_T>(0.0);
	for (unsigned long long int iteration = begin; iteration < end; iteration += step) {
		if (cs.cancelled()) break;
#ifdef ENABLE_DELAY
		mywait(DelaySeconds);
#endif
		const IDouble_T value = (IDouble_T)(iteration % 2)*static_cast<IDouble_T>(0.0012) + static_cast<IDouble_T>(0.0000003)*(IDouble_T)iteration;
...
auto sf = [](const unsigned long long int iteration, SDouble_T& sumatorio) {
#ifdef ENABLE_DELAY
	mywait(DelaySeconds);
#endif
	const IDouble_T value = (IDouble_T)(iteration % 2)*static_cast<IDouble_T>(0.0012) + static_cast<IDouble_T>(0.0000003)*(IDouble_T)iteration;
...
int main(int argc, char *argv[]) {
	...
	SDouble_T sumatorio = {3.123124, spec_double_precision};
