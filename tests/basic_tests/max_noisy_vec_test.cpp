...
///
/// \file     max_noisy_vec_test.cpp
/// \brief    Like max_vec_test.cpp but we speculate on the whole vector and it is noisy
...
bool lambda_test()
{
  const auto loop_f = [&](const size_t iteration, int& result, NoisyVector<int>& v_result) {
#ifdef ENABLE_DELAY
    mywait(DelaySeconds);
#endif
  ...
  const bool test_ok = bench(0, N, 1, loop_f, reset_result, test_f, avg_time, max_spec, v_result);
