...
///
/// \file     maxmin_vec_test_rev.cpp
/// \brief    Test based on finding the largest and the smallest integers in a vector and storing in a vector the largest element found up to that position (reverse iteration)
...
bool lambda_test()
{
  ...
  const auto loop_f = [&](const long long int iteration, std::vector<int>& minmax, SpecLib::SpecConsecVector<int>& v_result) {
#ifdef ENABLE_DELAY
    mywait(DelaySeconds);
#endif
  ...
  const bool test_ok = bench(N-1, -1LL, -1LL, loop_f, reset_result, test_f, avg_time, minmax, SpecLib::SpecConsecVector<int>(v_result));
...
static inline void sf(const long long int iteration, std::vector<int>& minmax, SpecLib::SpecConsecVector<int>& v_result)
{
#ifdef ENABLE_DELAY
  mywait(DelaySeconds);
#endif
...
bool sf_test()
{
  ...
  const bool test_ok = bench(N-1, -1LL, -1LL, sf, reset_result, test_f, avg_time, minmax, SpecLib::SpecConsecVector<int>(v_result));
