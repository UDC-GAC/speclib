...
///
/// \file     max_vec_test.cpp
/// \brief    Test based on finding the largest integer in a vector and storing in a vector the largest element found up to that position
...
bool lambda_test()
{
  const auto loop_f = [&](const size_t iteration, int& result, SpecLib::SpecConsecVector<int>& v_result) {
#ifdef ENABLE_DELAY
    mywait(DelaySeconds);
#endif
  ...
  const bool test_ok = bench(0, N, 1, loop_f, reset_result, test_f, avg_time, max_spec, SpecLib::SpecConsecVector<int>(v_result));
...
static inline void sf(const size_t iteration, int& result, SpecLib::SpecConsecVector<int>& v_result)
{
#ifdef ENABLE_DELAY
    mywait(DelaySeconds);
#endif
...
bool sf_test()
{
  ...
  const bool test_ok = bench(0, N, 1, sf, reset_result, test_f, avg_time, max_spec, SpecLib::SpecConsecVector<int>(v_result));
